...
New Flags:
	BJUMPER_PREGHAND_MULTIBIRTH //number of extra wombs to add into birth scene, undefined = single birth, 0 indicates all multi births processed and can go to birth scene
	
*/
	
	public class BoredJumperPregnancyHandler extends BasePregnancyHandler
	{
		public function set alwaysImpregnate(always:Boolean):void { _alwaysImpregnate = always; }
		public function set pregnancyQuantityMinimum(minQty:int):void { _pregnancyQuantityMinimum = minQty; }
		public function set pregnancyQuantityMaximum(maxQty:int):void { _pregnancyQuantityMaximum = maxQty; }
		
		public function BoredJumperPregnancyHandler() 
		{
			_handlesType = "BoredJumperPregnancy";
			_basePregnancyIncubationTime = (60 * 24 * 182); // 182 days
			_basePregnancyChance = .4;
			...
			_onSuccessfulImpregnation = boredJumperSuccessfulImpregnation;
			_onDurationEnd = boredJumperOnDurationEnd;
			...
			for(var i:int = 0; i <= 182; i += 1)
			{
				...
				{					
					addStageProgression(_basePregnancyIncubationTime - (i * 24 * 60), function(pregSlot:int):void {
						...
						AddLogEvent("Another internal upheaval. Fuck, where's the nearest trash bin? You could look past a foul toilet… Nothing for it: you lose your meal this time" + bonusText31 + ".\n\nThe mood swings have been getting worse. When you stubbed your toe a few days ago you could have hurt someone with scant provocation, only to revert to a pseudo-cathartic state of mind that" + bonusText312 + ". When you look down, your stomach is still bloated, blocking view of your crotch and the ground.\n\nIt's swollen, and all too upset.", "passive");
				...
				else if(i == 85)
				{
					addStageProgression(_basePregnancyIncubationTime - (i * 24 * 60), function(pregSlot:int):void {
						...
						kGAMECLASS.pc.createPerk("Musky Pheromones", 0, 0, 0, 0, "Boosts tease attack and arousal.");
						...
						if (kGAMECLASS.pc.findEmptyPregnancySlot(1) >= 0)
						{
							...
							bonusText853 = " Maybe you could go and get her to get a few more babies in there?";
						}
						else
						{
							bonusText852 = " Those cradles are full, sadly, but you'll be ready when you can fill 'em again.";
							bonusText853 = " Maybe when you carry your current litter to term she'll still be there with days-worth of pent-up seed to restart your journey?";
						}
		...
		public static function boredJumperSuccessfulImpregnation(father:Creature, mother:Creature, pregSlot:int, thisPtr:BasePregnancyHandler):void
		{
			...
			else if (pregSlot == 2)	kGAMECLASS.flags["BJUMPER_PC_PREG_TYPE2"] = father.cocks[0].cType;
			
			BasePregnancyHandler.defaultOnSuccessfulImpregnation(father, mother, pregSlot, thisPtr, undefined);
		}
		
		public static function boredJumperOnDurationEnd(mother:Creature, pregSlot:int, thisPtr:BasePregnancyHandler):void
		{
			var pData:PregnancyData = mother.pregnancyData[pregSlot];
			
				var jumperType:int;
				if (pregSlot == 0) jumperType = kGAMECLASS.flags["BJUMPER_PC_PREG_TYPE"];
				else if (pregSlot == 1)	jumperType = kGAMECLASS.flags["BJUMPER_PC_PREG_TYPE1"];
				else if (pregSlot == 2)	jumperType = kGAMECLASS.flags["BJUMPER_PC_PREG_TYPE2"];
				...
				// Generate babies
				var babyList:Array = (new BoredJumperPregnancyHandler()).boredJumperChildren(mother, babies, jumperType);
				...
				else
				{
					...
					kGAMECLASS.flags["BJUMPER_PREGHAND_MULTIBIRTH"] -= 1;
				}
				//create status effects unique to each womb so they can be used together if needed
				if (pregSlot == 0) mother.createStatusEffect("Bored Jumper Pregnancy Ends 0", babies, belly, pregSlot, babyGender, true);
				if (pregSlot == 1) mother.createStatusEffect("Bored Jumper Pregnancy Ends 1", babies, belly, pregSlot, babyGender, true);
				if (pregSlot == 2) mother.createStatusEffect("Bored Jumper Pregnancy Ends 2", babies, belly, pregSlot, babyGender, true);
				//if this is a single womb birth or we have processed all the extra wombs, we can trigger the birth scene
				if (kGAMECLASS.flags["BJUMPER_PREGHAND_MULTIBIRTH"] == undefined || kGAMECLASS.flags["BJUMPER_PREGHAND_MULTIBIRTH"] == 0) kGAMECLASS.eventQueue.push(kGAMECLASS.boredJumperPregPregnancyEnds);
				//clean up all the preg variables and flags each time
				BoredJumperPregnancyHandler.boredJumperCleanupData(mother, pregSlot, thisPtr);
		}
		public static function boredJumperCleanupData(mother:Creature, pregSlot:int, thisPtr:BasePregnancyHandler):void
		{
			...
			if (pregSlot == 0)
			{
				jumperType = kGAMECLASS.flags["BJUMPER_PC_PREG_TYPE"];
				kGAMECLASS.flags["BJUMPER_PC_PREG_TYPE"] = undefined;
			}
			else if (pregSlot == 1)
			{
				jumperType = kGAMECLASS.flags["BJUMPER_PC_PREG_TYPE1"];
				kGAMECLASS.flags["BJUMPER_PC_PREG_TYPE1"] = undefined;
			}
			else if (pregSlot == 2)
			{
				jumperType = kGAMECLASS.flags["BJUMPER_PC_PREG_TYPE2"];
				kGAMECLASS.flags["BJUMPER_PC_PREG_TYPE2"] = undefined;
			}
			...
			{
				if (kGAMECLASS.flags["BJUMPER_HORSE_TOTAL_KIDS"] == undefined) kGAMECLASS.flags["BJUMPER_HORSE_TOTAL_KIDS"] = 0;
				kGAMECLASS.flags["BJUMPER_HORSE_TOTAL_KIDS"] += pData.pregnancyQuantity;
				StatTracking.track("pregnancy/jumper horse births", pData.pregnancyQuantity);
				IncrementFlag("BJUMPER_HORSE_PC_PREG_NEW");
			}
			else if (jumperType == GLOBAL.TYPE_FELINE)
			{
				if (kGAMECLASS.flags["BJUMPER_CAT_TOTAL_KIDS"] == undefined) kGAMECLASS.flags["BJUMPER_CAT_TOTAL_KIDS"] = 0;
				kGAMECLASS.flags["BJUMPER_CAT_TOTAL_KIDS"] += pData.pregnancyQuantity;
				StatTracking.track("pregnancy/jumper cat births", pData.pregnancyQuantity);
				IncrementFlag("BJUMPER_CAT_PC_PREG_NEW");
			}
			else if (jumperType == GLOBAL.TYPE_CANINE)
			{
				if (kGAMECLASS.flags["BJUMPER_DOG_TOTAL_KIDS"] == undefined) kGAMECLASS.flags["BJUMPER_DOG_TOTAL_KIDS"] = 0;
				kGAMECLASS.flags["BJUMPER_DOG_TOTAL_KIDS"] += pData.pregnancyQuantity;
				StatTracking.track("pregnancy/jumper dog births", pData.pregnancyQuantity);
				IncrementFlag("BJUMPER_DOG_PC_PREG_NEW");
			}
			else 
			{
				if (kGAMECLASS.flags["BJUMPER_TERRAN_TOTAL_KIDS"] == undefined) kGAMECLASS.flags["BJUMPER_TERRAN_TOTAL_KIDS"] = 0;
				kGAMECLASS.flags["BJUMPER_TERRAN_TOTAL_KIDS"] += pData.pregnancyQuantity;
				StatTracking.track("pregnancy/jumper terran births", pData.pregnancyQuantity);
				IncrementFlag("BJUMPER_TERRAN_PC_PREG_NEW");
			}

			StatTracking.track("pregnancy/total births", pData.pregnancyQuantity);
			StatTracking.track("pregnancy/total day care", pData.pregnancyQuantity);
			
			if (kGAMECLASS.flags["BJUMPER_PC_PREG_TYPE"] == undefined && kGAMECLASS.flags["BJUMPER_PC_PREG_TYPE1"] == undefined && kGAMECLASS.flags["BJUMPER_PC_PREG_TYPE2"] == undefined) kGAMECLASS.pc.removePerk("Musky Pheromones");
			
			kGAMECLASS.flags["BJUMPER_PREG_SCENE_TYPE"] = jumperType;
		...
		override public function nurseryEndPregnancy(mother:Creature, pregSlot:int, useBornTimestamp:uint):Child
		{
			...
			var babyList:Array = boredJumperChildren(mother, pData.pregnancyQuantity, jumperType);
			var i:int = 0;
			for(i = 0; i < babyList.length; i++)
			{
		...
		private function boredJumperChildren(mother:Creature, numKids:int = 0, jumperType:int=0):Array
		{
			var babyList:Array = [];
			var c:UniqueChild;
			
			//these flags are used in the birth scenes
			kGAMECLASS.flags["BJUMPER_PC_BIRTH_SCENE_FURCOLOR1"] = undefined;
			kGAMECLASS.flags["BJUMPER_PC_BIRTH_SCENE_FURCOLOR2"] = undefined;
			kGAMECLASS.flags["BJUMPER_PC_BIRTH_SCENE_HAIRCOLOR1"] = undefined;
			kGAMECLASS.flags["BJUMPER_PC_BIRTH_SCENE_HAIRCOLOR2"] = undefined;
			kGAMECLASS.flags["BJUMPER_PC_BIRTH_SCENE_BOYS"] = 0;
			kGAMECLASS.flags["BJUMPER_PC_BIRTH_SCENE_GIRLS"] = 0;
			kGAMECLASS.flags["BJUMPER_PC_BIRTH_SCENE_HERMS"] = 0;
			var kidSex:int;
			...
			{
				kidSex = rand(3);
				if(kidSex == 0) kGAMECLASS.flags["BJUMPER_PC_BIRTH_SCENE_BOYS"] += 1;
				else if(kidSex == 1) kGAMECLASS.flags["BJUMPER_PC_BIRTH_SCENE_GIRLS"] += 1;
				else kGAMECLASS.flags["BJUMPER_PC_BIRTH_SCENE_HERMS"] += 1;
			}
			...
			for(i = 0; i < 3; i++)
			{
				...
				if (i == 0)
				{
					if (kGAMECLASS.flags["BJUMPER_PC_BIRTH_SCENE_BOYS"] > 0) skipSex = false;
				}
				else if (i == 1)
				{
					if (kGAMECLASS.flags["BJUMPER_PC_BIRTH_SCENE_GIRLS"] > 0) skipSex = false;
				}
				else if (i == 2)
				{
					if (kGAMECLASS.flags["BJUMPER_PC_BIRTH_SCENE_HERMS"] > 0) skipSex = false;
				}
				...
				if (!skipSex)
				{
					...
					if (i == 0) c.NumMale += kGAMECLASS.flags["BJUMPER_PC_BIRTH_SCENE_BOYS"];
					else if (i == 1) c.NumFemale += kGAMECLASS.flags["BJUMPER_PC_BIRTH_SCENE_GIRLS"];
					else if (i == 2) c.NumIntersex += kGAMECLASS.flags["BJUMPER_PC_BIRTH_SCENE_HERMS"];
					...
					//used in the birth scenes to display colors of the kids
					if (kGAMECLASS.flags["BJUMPER_PC_BIRTH_SCENE_FURCOLOR1"] == undefined) kGAMECLASS.flags["BJUMPER_PC_BIRTH_SCENE_FURCOLOR1"] = c.furColor;
					else if (kGAMECLASS.flags["BJUMPER_PC_BIRTH_SCENE_FURCOLOR1"] != c.furColor) kGAMECLASS.flags["BJUMPER_PC_BIRTH_SCENE_FURCOLOR2"] = c.furColor;
					if (kGAMECLASS.flags["BJUMPER_PC_BIRTH_SCENE_HAIRCOLOR1"] == undefined) kGAMECLASS.flags["BJUMPER_PC_BIRTH_SCENE_HAIRCOLOR1"] = c.hairColor;
					else if (kGAMECLASS.flags["BJUMPER_PC_BIRTH_SCENE_HAIRCOLOR1"] != c.hairColor) kGAMECLASS.flags["BJUMPER_PC_BIRTH_SCENE_HAIRCOLOR2"] = c.hairColor;
				}
			}
		...
		override public function tryKnockUp(father:Creature, mother:Creature, pregSlot:int):Boolean
		{
			...
			//turn off heat sex which makes preg chance 100% and taur loss sex which increases kid count
			kGAMECLASS.flags["BJUMPER_HEAT_SEX"] = undefined;
			kGAMECLASS.flags["BJUMPER_TAUR_LOSS_SEX"] = undefined;
