...
public function showPregBoredJumper(nude:Boolean = false,preg:Boolean = false,jumperType:int = 0):void
{
	...
	showName("BORED\nJUMPER");
	if(jumperType == GLOBAL.TYPE_EQUINE) showBust("BORED_JUMPER_EQUINE" + (preg ? "_PREG" : "") + (nude ? "_NUDE" : ""));
	else if(jumperType == GLOBAL.TYPE_FELINE) showBust("BORED_JUMPER_FELINE" + (preg ? "_PREG" : "") + (nude ? "_NUDE" : ""))
	else if(jumperType == GLOBAL.TYPE_CANINE) showBust("BORED_JUMPER_CANINE" + (preg ? "_PREG" : "") + (nude ? "_NUDE" : ""))
	else showBust("BORED_JUMPER_TERRAN" + (preg ? "_PREG" : "") + (nude ? "_NUDE" : ""))
	author("William");
}
...
public function boredJumperPregEncounterJumperPreg():Boolean
{
	...
	if (firstTime)
	{
		...
		output(". Oof, that's great. Got a really bad headache...”</i> She presses her back to the wall, grunts, and slumps down, clutching her visibly rounded tummy. Her taut and ropy muscles have disappeared under the heft of a rounding baby bulge. <i>“F-fuck, so heavy…”</i> she murmurs. Looks like she's an expecting mother, and considering the potent fragrance coming off her, she's no doubt bearing <i>your children.</i> Thinking about it, she smelled like a long-stewed fuckbunny before… but now her aroma is magnified by the weight in her womb.");
...
public function boredJumperPregEncounterSteelePreg():Boolean
{
	...
	if (firstTime)
	{
		...
		output("\n\n<i>“Wow, they're kicking already! You've gotta have a lot of kids in there, huh?”</i> <i>Thump.</i> <i>“Oof, that was a strong one! They're sure growing fast. Wonder if that's 'cause 'a you or me?”</i> she squeals and giggles");
		if (pc.isTaur())
		{
...
public function boredJumperPregEncounterGoWithRestRvsCowgirlNext(arg:Array):void
{
	...
	if (sceneNum >= 2) output(" baby-filled");
...
public function boredJumperPregPregnancyEnds():void
{
	...
	{
		output("You've gotten wetter since this all started, but something's just happened that redefines your concept of being drenched. You place a protective hand to your [pc.belly], and note for the first time that the distension has begun to abate. You're flooding between the legs... Oh shit…");
